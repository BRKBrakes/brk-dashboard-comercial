...
async function loadTopKam() {
  ...
  // Una sola llamada trae todos los KAMs, luego se agrupa en frontend
  const detAll = await rpc('dash_recuperacion', { p_token: TOKEN, p_kam: null });
  const detalles = r.data.map(k => {
    if (!detAll.ok) return { ok: false };
    const nombreNorm = (k.nombre||'').toUpperCase().trim();
    return {
      ok: true,
      cayendo: (detAll.cayendo||[]).filter(c => (c.vendedor||'').toUpperCase().trim() === nombreNorm),
      sin_compra_60d: (detAll.sin_compra_60d||[]).filter(c => (c.vendedor||'').toUpperCase().trim() === nombreNorm)
    };
  });
...
async function renderOkrKam(el, opcionesMeses, mesActual) {
  ...
  if (!OKRKAM_KAM) {
    ...
    function gaugeKrKam(kam, cumplidos, total) {
      ...
      return `<div style="text-align:center;min-width:170px;padding:0 12px;">
        <svg viewBox="0 0 120 70" width="160" height="96">
          <path d="M10,65 A50,50 0 0,1 110,65" fill="none" stroke="#333" stroke-width="10" stroke-linecap="round"/>
          <path d="M10,65 A50,50 0 0,1 110,65" fill="none" stroke="${color}" stroke-width="10" stroke-linecap="round" stroke-dasharray="${pct*157} 157"/>
        ...
        <div style="font-size:11px;color:var(--text-dim);margin-top:2px;">${esc(titleCase(kam))}</div>
      </div>`;
    }

    html += `<div class="card">
      <h2>Cumplimiento de KR por KAM (${label})</h2>
      <div style="display:flex;gap:32px;flex-wrap:wrap;justify-content:center;padding:8px 0;">
        ${kamsDisponibles.filter(kam => kam !== 'GOMEZ RODRIGUEZ CARLOS ANDRES').map(kam => {
          ...
          // KR2 Tipo A
          total++; if (t.venta2025 && ((t.venta2026-t.venta2025)/t.venta2025) >= 0.10) cumplidos++;
          // KR3 Clientes Nuevos
          total++; if (metaCNK && cnTotalK >= metaCNK) cumplidos++;
          // O2-KR1 Cartera <= 2.5%
    ...
    html += `<div class="card">
      ...
      <table>
        ...
        ${(() => {
          const totCn = clientesNuevos.reduce((s,c) => s + (c.venta_periodo||0), 0);
          const totFact = presupuesto.reduce((s,p) => s + (p.facturado||0), 0);
    ...
    mesesAct.forEach(m => {
      ...
      const colorM = promDso === null ? 'var(--text-dim)' : promDso <= 60 ? '#4ade80' : promDso <= 75 ? '#ff9f43' : '#ff6b6b';
      html += `<td class="num" style="color:${colorM};">${promDso!==null?promDso+'d':'Sin cerrar'}</td>`;
    });
    html += `</tr></table></div></div>`;
...
const MAPEO_CARTERA = {
  ...
  'Nro. docto. cruce': 'nro_documento_cruce', 'Cond. pago factura': 'cond_pago_cliente',
...
async function mostrarFacturasCarteraMulti() {
  ...
  if (btnExportar) {
    btnExportar.addEventListener('click', () => {
      if (!facturas.length) { alert('No hay facturas en la selección actual.'); return; }
      const facturasOrdenadas = [...facturas].sort((a,b) => (b.dias_real_vencido||0) - (a.dias_real_vencido||0));
      const filas = facturasOrdenadas.map(f => ({
        'Sucursal': f.sucursal || '',
        ...
        'Estado': (f.dias_real_vencido||0) >= 1 ? 'Vencida' : 'No vencida'
      }));
...
async function cargarExcluidas() {
  ...
  return (r.ok && Array.isArray(r.excluidas)) ? r.excluidas : [];
}

async function guardarExcluidas(arr) {
  return await rpc('dash_exclusiones_guardar', { p_token: TOKEN, p_excluidas: arr });
}
...
async function loadNps() {
  ...
  <div class="card" style="margin-bottom:16px;display:flex;gap:24px;align-items:center;flex-wrap:wrap;">
    <div style="flex:1;min-width:260px;">
      ...
      <a href="https://wa.me/?text=${encodeURIComponent('Queremos saber cómo ha sido tu experiencia con BRK Colombia. Tu retroalimentación es fundamental para seguir mejorando el servicio que ofrecemos a nuestros aliados.\n— Carlos Gómez, Líder BRK Colombia\n\n' + NPS_URL)}" target="_blank"
        style="display:inline-flex;align-items:center;gap:8px;background:#25D366;color:#fff;padding:10px 20px;border-radius:6px;font-weight:700;font-size:13px;letter-spacing:1px;text-decoration:none;text-transform:uppercase;">
  ...
  ${porKam.length ? `<div class="card" style="margin-bottom:16px;">
    ...
    <table>
      <tr>
        ...
        <th class="num">Atención</th>
        <th class="num">Tiempos</th>
        <th class="num">Calidad</th>
      ...
      ${(() => {
        ...
        return `<tr style="font-weight:700;border-top:2px solid var(--neon);">
          ...
          <td class="num" data-val="${wavg('tiempos')}" style="color:${nc(wavg('tiempos'))};">${wavg('tiempos')}</td>
          <td class="num" data-val="${wavg('calidad')}" style="color:${nc(wavg('calidad'))};">${wavg('calidad')}</td>
          <td class="num" data-val="${wavg('cotizacion')}" style="color:${nc(wavg('cotizacion'))};">${wavg('cotizacion')}</td>
          <td class="num" data-val="${npsT}" style="color:${npsC};">${npsT}</td>
        </tr>`;
      })()}
    </table>
  </div>` : ''}
